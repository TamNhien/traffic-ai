...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.13</span></div>
    ...
    <main>
      ...
      <section className="line-layout" id="cameras">
        <article className="panel line-panel"><div className="panel-head"><div><span className="panel-kicker">ROAD ZONE + COUNTING LINE</span><h2>Khoanh lòng đường và đặt vạch đếm</h2></div><span className={`lock-state ${activePipeline ? 'locked' : ''}`}>{activePipeline ? 'Đang khóa khi AI chạy' : 'Có thể chỉnh'}</span></div>
          ...
          <p className="hint"><strong>Vùng xanh = lòng đường được phép đếm.</strong> V0.5.13 tiếp tục khóa cứng hai đầu vạch vàng ở bên trong vùng xanh và từ chối polygon bị bắt chéo. Khi đếm, cả điểm track trước và sau lúc crossing cũng phải nằm trong lòng đường; xe trên lề/vỉa hè không được cộng IN/OUT.</p>
        </article>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.13</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.13 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
