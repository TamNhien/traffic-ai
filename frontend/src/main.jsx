...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.29</span></div>
...
function GroundTruthBenchmark({ selectedCameraId, sessions }) {
  ...
  return <section className="benchmark-layout" id="benchmark">
    <article className="panel benchmark-panel">
      <div className="panel-head"><div><span className="panel-kicker">GROUND-TRUTH COUNTING BENCHMARK</span><h2>Đánh dấu xe thật cắt vạch trên chính clip</h2></div><span className="lock-state">V0.5.29</span></div>
      <p className="hint">Benchmark không đoán tổng xe. Bạn xem lại clip, dừng đúng lúc mỗi xe <strong>thực sự cắt vạch</strong> rồi bấm đánh dấu IN/OUT. Traffic AI lưu timecode video cho từng event và dùng <strong>global temporal matching</strong> để ghép GT ↔ AI theo thứ tự thời gian, không dùng class để ép ghép; nhờ đó các cụm xe cắt vạch sát nhau ít tạo báo cáo sai cặp hơn.</p>
    ...
    <article className="panel benchmark-report-panel">
      ...
      {report ? <>
        ...
        <div className="benchmark-diff-list">{report.class_mismatch_items?.length ? report.class_mismatch_items.map(item=><button key={`c-${item.ground_truth_id}-${item.ai_event_id}`} className="diff-row false-positive" onClick={()=>seekTo(item.time)}><strong>{formatVideoTime(item.time)}</strong><span>{String(item.direction).toUpperCase()} · GT {vehicleLabels[item.ground_truth_vehicle_type] || item.ground_truth_vehicle_type}</span><em>AI → {vehicleLabels[item.ai_vehicle_type] || item.ai_vehicle_type}</em></button>) : <div className="empty">Không có event đã khớp thời gian nhưng sai loại xe.</div>}</div>
    ...
    <main>
      ...
      <section className="content-grid" id="live">
        <article className="panel camera-panel">
          ...
          {activePipeline && ((activePipeline.bicycle_class_rescues ?? 0) > 0 || (activePipeline.truck_class_rescues ?? 0) > 0) && <div className="source-status ok"><strong>✓ Class Refiner đã sửa class mục tiêu</strong><span>Xe đạp cứu {activePipeline.bicycle_class_rescues ?? 0} · Xe tải cứu {activePipeline.truck_class_rescues ?? 0}. Refiner chỉ nhận detection chồng/bao đúng track mục tiêu, không lấy nhầm xe máy đậu cạnh crop.</span></div>}
          ...
          {latestPipeline && !activePipeline && <div className="pipeline-result">Lần chạy gần nhất: <strong>{latestPipeline.status}</strong> · {latestPipeline.processed_frames} frame · worker {latestPipeline.total_count} crossing · DB mới {latestPipeline.persisted_events ?? latestPipeline.delivered_events ?? 0} event · dedup {latestPipeline.deduplicated_events ?? 0} · Class refine {latestPipeline.class_refine_checks ?? 0} · Refiner chung {latestPipeline.general_refine_checks ?? 0} · Consensus {latestPipeline.class_consensus_rescues ?? 0} · Xe đạp cứu {latestPipeline.bicycle_class_rescues ?? 0} · Xe đạp thấy {latestPipeline.bicycle_tracks_seen ?? 0} · Xe tải cứu {latestPipeline.truck_class_rescues ?? 0} · Xe tải thấy {latestPipeline.truck_tracks_seen ?? 0} · Xe tải cắt vạch {latestPipeline.truck_crossing_tracks ?? 0} · Xe lớn anchor {latestPipeline.heavy_anchor_tracks ?? 0} · Xe lớn cứu center {latestPipeline.heavy_center_rescues ?? 0} · Nối track xe lớn {latestPipeline.heavy_stitch_recoveries ?? 0} · Gộp car/truck {latestPipeline.four_wheel_duplicate_suppressed ?? 0} · Video-start {latestPipeline.video_start_rescues ?? 0} · Human Guard {latestPipeline.human_guard_rejections ?? 0} · Rider giữ {latestPipeline.rider_guard_rescues ?? 0} · Guard xác nhận {latestPipeline.human_guard_deferred_commits ?? 0} · Guard timeout {latestPipeline.human_guard_pending_drops ?? 0}{latestPipeline.last_error ? ` · ${latestPipeline.last_error}` : ''}</div>}
        </article>
        <article className="panel"><div className="panel-head"><div><span className="panel-kicker">VEHICLE COUNT</span><h2>Theo loại phương tiện · phiên hiện tại</h2></div></div><div className="counting-summary"><div><span>Tổng lượt cắt vạch</span><strong>{sessionPipeline?.total_count ?? 0}</strong></div><div><span>IN</span><strong>{sessionPipeline?.in_count ?? 0}</strong></div><div><span>OUT</span><strong>{sessionPipeline?.out_count ?? 0}</strong></div></div><div className="crossing-quality"><span>Trực tiếp <strong>{sessionPipeline?.direct_crossings ?? 0}</strong></span><span>Nội suy <strong>{sessionPipeline?.interpolated_crossings ?? 0}</strong></span><span>Cứu qua gap <strong>{sessionPipeline?.rescued_crossings ?? 0}</strong></span><span>Fast-confirm <strong>{sessionPipeline?.fast_confirm_rescues ?? 0}</strong></span><span>Bracket-confirm <strong>{sessionPipeline?.bracket_confirm_rescues ?? 0}</strong></span><span>Rescue bị loại <strong>{sessionPipeline?.rescue_validation_rejections ?? 0}</strong></span><span>Road Zone loại <strong>{sessionPipeline?.rejected_outside_road ?? 0}</strong></span><span>Class refine <strong>{sessionPipeline?.class_refine_checks ?? 0}</strong></span><span>Refiner miền <strong>{sessionPipeline?.domain_refine_checks ?? 0}</strong></span><span>Refiner chung <strong>{sessionPipeline?.general_refine_checks ?? 0}</strong></span><span>Consensus <strong>{sessionPipeline?.class_consensus_rescues ?? 0}</strong></span><span>Xe đạp cứu <strong>{sessionPipeline?.bicycle_class_rescues ?? 0}</strong></span><span>Xe đạp thấy <strong>{sessionPipeline?.bicycle_tracks_seen ?? 0}</strong></span><span>Xe tải cứu <strong>{sessionPipeline?.truck_class_rescues ?? 0}</strong></span><span>Xe tải thấy <strong>{sessionPipeline?.truck_tracks_seen ?? 0}</strong></span><span>Xe tải cắt vạch <strong>{sessionPipeline?.truck_crossing_tracks ?? 0}</strong></span><span>Xe lớn anchor <strong>{sessionPipeline?.heavy_anchor_tracks ?? 0}</strong></span><span>Xe lớn cứu center <strong>{sessionPipeline?.heavy_center_rescues ?? 0}</strong></span><span>Nối track xe lớn <strong>{sessionPipeline?.heavy_stitch_recoveries ?? 0}</strong></span><span>Gộp car/truck <strong>{sessionPipeline?.four_wheel_duplicate_suppressed ?? 0}</strong></span><span>Video-start <strong>{sessionPipeline?.video_start_rescues ?? 0}</strong></span><span>Human Guard <strong>{sessionPipeline?.human_guard_rejections ?? 0}</strong></span><span>Rider giữ <strong>{sessionPipeline?.rider_guard_rescues ?? 0}</strong></span><span>Guard chờ <strong>{sessionPipeline?.human_guard_pending_crossings ?? 0}</strong></span><span>Guard xác nhận <strong>{sessionPipeline?.human_guard_deferred_commits ?? 0}</strong></span><span>Guard timeout <strong>{sessionPipeline?.human_guard_pending_drops ?? 0}</strong></span></div><div className="vehicle-list">{vehicleRows.map(row => <div className="vehicle-row" key={row.label}><span>{row.label}</span><strong>{row.value}</strong></div>)}</div>{(sessionPipeline?.truck_tracks_seen ?? 0) > 0 && (sessionPipeline?.counts_by_type?.truck ?? 0) === 0 && <div className="source-status"><strong>ℹ AI đã thấy xe tải nhưng chưa ghi event xe tải</strong><span>Đã thấy {sessionPipeline?.truck_tracks_seen ?? 0} track xe tải; geometry đã phát crossing cho {(sessionPipeline?.truck_crossing_tracks ?? 0)} track. V0.5.29 còn theo dõi center của xe bốn bánh, nối lại ID van qua gap dài hơn và gộp box car/truck/bus trùng nhau. Nếu “Xe tải cắt vạch” vẫn bằng 0 sau khi Heavy-center+ tăng, gửi lại benchmark/snapshot để khoanh đúng tầng semantic event.</span></div>}<p className="hint"><strong>V0.5.29</strong> giữ <strong>Video-Origin + Heavy-Vehicle Gate Rescue</strong> và nâng thành <strong>Heavy Track Fusion + Center-Gate Rescue</strong>: box CAR/TRUCK/BUS chồng nhau được alias về một xe, van được nối track qua gap dài hơn, và center trajectory chỉ được dùng như cổng cứu thứ hai khi strict anchor gate không phát event. Bicycle consensus chuyển sang chế độ precision: cần ít nhất 3 frame độc lập, có confidence mạnh tối thiểu và phải thắng bằng chứng motorcycle theo margin trước khi BIKE+ được giữ. <strong>Xe tải thấy</strong> là detection/track; <strong>Xe tải cắt vạch</strong> là geometry; <strong>Xe tải</strong> vẫn chỉ là event đã lưu. Crossing Engine 7.2 + Transactional Human Guard 2.1 tiếp tục giữ nguyên.</p></article>
      </section>
      ...
      <section className="line-layout" id="cameras">
        <article className="panel line-panel"><div className="panel-head"><div><span className="panel-kicker">ROAD ZONE + COUNTING LINE</span><h2>Khoanh lòng đường và đặt vạch đếm</h2></div><span className={`lock-state ${activePipeline ? 'locked' : ''}`}>{activePipeline ? 'Đang khóa khi AI chạy' : 'Có thể chỉnh'}</span></div>
          ...
          <p className="hint"><strong>Vùng xanh = lòng đường được phép đếm.</strong> V0.5.29 giữ <strong>Hybrid Recall</strong> + <strong>Auto Road-Zone</strong>, <strong>Single-Object Guard</strong> và <strong>Transactional Human Guard 2.1</strong> để BUS/TRUCK chồng box chỉ được tính là một phương tiện: khi AI chạy đủ track chuyển động, bấm “AI đề xuất theo luồng xe” để hệ thống tự khoanh phần đường xe thật sự đi qua và đặt vạch gần vuông góc luồng xe. Hybrid Recall: YOLO26 pretrained quét toàn khung để tạo track ổn định; best.pt kiểm tra class theo miền và YOLO26m cung cấp second-opinion target-aware trước khi consensus quyết định class hiếm. Detection chưa có ID vẫn hiện box vàng; chỉ track cắt vạch vàng hợp lệ trong vùng xanh mới được cộng IN/OUT.</p>
        </article>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.29</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.29 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
        ...
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">FINE-TUNE</span><h2>Huấn luyện YOLO26 tùy biến</h2></div><span className="lock-state">Fresh run</span></div>
          ...
          <p className="hint">Sau khi kích hoạt <code>best.pt</code>, V0.5.29 dùng <strong>Hybrid Recall + Dual Refiner</strong>: YOLO26 pretrained đảm nhiệm detect/track, <code>best.pt</code> refine theo miền và YOLO26m làm second-opinion; consensus nhiều frame chỉ sửa class khi bằng chứng target-matched đủ mạnh. Strict Gate/Road Guard vẫn quyết định đếm; Annotation Studio tiếp tục dùng để nâng chất lượng ground truth cho các lần train sau.</p>
        </article>
      </section>
