...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.26</span></div>
...
function GroundTruthBenchmark({ selectedCameraId, sessions }) {
  ...
  return <section className="benchmark-layout" id="benchmark">
    <article className="panel benchmark-panel">
      <div className="panel-head"><div><span className="panel-kicker">GROUND-TRUTH COUNTING BENCHMARK</span><h2>Đánh dấu xe thật cắt vạch trên chính clip</h2></div><span className="lock-state">V0.5.26</span></div>
    ...
    <main>
      ...
      <section className="content-grid" id="live">
        <article className="panel camera-panel">
          ...
          {activePipeline && (activePipeline.detections_current_frame ?? 0) > 0 && (activePipeline.active_tracks ?? 0) === 0 && <div className="source-status bad"><strong>⚠ YOLO thấy xe nhưng ByteTrack chưa cấp ID</strong><span>V0.5.26 vẫn vẽ box DET màu vàng cho detection chưa có ID. Bộ đếm chỉ tăng khi track ổn định; profile ByteTrack high-recall sẽ cố bám các xe nhỏ/nhanh ở những frame tiếp theo.</span></div>}
          ...
          {latestPipeline && !activePipeline && <div className="pipeline-result">Lần chạy gần nhất: <strong>{latestPipeline.status}</strong> · {latestPipeline.processed_frames} frame · worker {latestPipeline.total_count} crossing · DB mới {latestPipeline.persisted_events ?? latestPipeline.delivered_events ?? 0} event · dedup {latestPipeline.deduplicated_events ?? 0} · Class refine {latestPipeline.class_refine_checks ?? 0} · Xe đạp cứu {latestPipeline.bicycle_class_rescues ?? 0} · Xe tải cứu {latestPipeline.truck_class_rescues ?? 0} · Video-start {latestPipeline.video_start_rescues ?? 0} · Human Guard {latestPipeline.human_guard_rejections ?? 0} · Rider giữ {latestPipeline.rider_guard_rescues ?? 0} · Guard xác nhận {latestPipeline.human_guard_deferred_commits ?? 0} · Guard timeout {latestPipeline.human_guard_pending_drops ?? 0}{latestPipeline.last_error ? ` · ${latestPipeline.last_error}` : ''}</div>}
        </article>
        <article className="panel"><div className="panel-head"><div><span className="panel-kicker">VEHICLE COUNT</span><h2>Theo loại phương tiện · phiên hiện tại</h2></div></div><div className="counting-summary"><div><span>Tổng lượt cắt vạch</span><strong>{sessionPipeline?.total_count ?? 0}</strong></div><div><span>IN</span><strong>{sessionPipeline?.in_count ?? 0}</strong></div><div><span>OUT</span><strong>{sessionPipeline?.out_count ?? 0}</strong></div></div><div className="crossing-quality"><span>Trực tiếp <strong>{sessionPipeline?.direct_crossings ?? 0}</strong></span><span>Nội suy <strong>{sessionPipeline?.interpolated_crossings ?? 0}</strong></span><span>Cứu qua gap <strong>{sessionPipeline?.rescued_crossings ?? 0}</strong></span><span>Fast-confirm <strong>{sessionPipeline?.fast_confirm_rescues ?? 0}</strong></span><span>Bracket-confirm <strong>{sessionPipeline?.bracket_confirm_rescues ?? 0}</strong></span><span>Rescue bị loại <strong>{sessionPipeline?.rescue_validation_rejections ?? 0}</strong></span><span>Class refine <strong>{sessionPipeline?.class_refine_checks ?? 0}</strong></span><span>Xe đạp cứu <strong>{sessionPipeline?.bicycle_class_rescues ?? 0}</strong></span><span>Xe tải cứu <strong>{sessionPipeline?.truck_class_rescues ?? 0}</strong></span><span>Video-start <strong>{sessionPipeline?.video_start_rescues ?? 0}</strong></span><span>Human Guard <strong>{sessionPipeline?.human_guard_rejections ?? 0}</strong></span><span>Rider giữ <strong>{sessionPipeline?.rider_guard_rescues ?? 0}</strong></span><span>Guard chờ <strong>{sessionPipeline?.human_guard_pending_crossings ?? 0}</strong></span><span>Guard xác nhận <strong>{sessionPipeline?.human_guard_deferred_commits ?? 0}</strong></span><span>Guard timeout <strong>{sessionPipeline?.human_guard_pending_drops ?? 0}</strong></span></div><div className="vehicle-list">{vehicleRows.map(row => <div className="vehicle-row" key={row.label}><span>{row.label}</span><strong>{row.value}</strong></div>)}</div><p className="hint"><strong>V0.5.26</strong> thêm <strong>Target-aware Class Refiner 3.0</strong>: best.pt chỉ được phép sửa class của đúng box mục tiêu, có thể cứu xe đạp bị YOLO26s gọi là xe máy và xe tải nhỏ bị gọi là car. Video local không còn bỏ crossing thật ở vài frame đầu; RTSP vẫn giữ startup guard. Crossing Engine 7.2 + Transactional Human Guard 2.1 tiếp tục giữ nguyên. Tổng xe vẫn là <strong>IN + OUT</strong>; telemetry không còn gọi mọi crossing có gap &gt; 1 frame là “cứu”. Chế độ Phát mượt dùng video native; AI vẫn detect/track/đếm ở nền.</p></article>
      </section>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.26</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.26 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
