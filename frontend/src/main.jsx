...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.24</span></div>
...
function GroundTruthBenchmark({ selectedCameraId, sessions }) {
  ...
  return <section className="benchmark-layout" id="benchmark">
    <article className="panel benchmark-panel">
      <div className="panel-head"><div><span className="panel-kicker">GROUND-TRUTH COUNTING BENCHMARK</span><h2>Đánh dấu xe thật cắt vạch trên chính clip</h2></div><span className="lock-state">V0.5.24</span></div>
    ...
    <main>
      ...
      <section className="content-grid" id="live">
        <article className="panel camera-panel">
          ...
          {latestPipeline && !activePipeline && <div className="pipeline-result">Lần chạy gần nhất: <strong>{latestPipeline.status}</strong> · {latestPipeline.processed_frames} frame · worker {latestPipeline.total_count} crossing · DB mới {latestPipeline.persisted_events ?? latestPipeline.delivered_events ?? 0} event · dedup {latestPipeline.deduplicated_events ?? 0} · Human Guard {latestPipeline.human_guard_rejections ?? 0} · Rider giữ {latestPipeline.rider_guard_rescues ?? 0}{latestPipeline.last_error ? ` · ${latestPipeline.last_error}` : ''}</div>}
        </article>
        <article className="panel"><div className="panel-head"><div><span className="panel-kicker">VEHICLE COUNT</span><h2>Theo loại phương tiện · phiên hiện tại</h2></div></div><div className="counting-summary"><div><span>Tổng lượt cắt vạch</span><strong>{sessionPipeline?.total_count ?? 0}</strong></div><div><span>IN</span><strong>{sessionPipeline?.in_count ?? 0}</strong></div><div><span>OUT</span><strong>{sessionPipeline?.out_count ?? 0}</strong></div></div><div className="crossing-quality"><span>Trực tiếp <strong>{sessionPipeline?.direct_crossings ?? 0}</strong></span><span>Nội suy <strong>{sessionPipeline?.interpolated_crossings ?? 0}</strong></span><span>Cứu qua gap <strong>{sessionPipeline?.rescued_crossings ?? 0}</strong></span><span>Fast-confirm <strong>{sessionPipeline?.fast_confirm_rescues ?? 0}</strong></span><span>Rescue bị loại <strong>{sessionPipeline?.rescue_validation_rejections ?? 0}</strong></span><span>Human Guard <strong>{sessionPipeline?.human_guard_rejections ?? 0}</strong></span><span>Rider giữ <strong>{sessionPipeline?.rider_guard_rescues ?? 0}</strong></span></div><div className="vehicle-list">{vehicleRows.map(row => <div className="vehicle-row" key={row.label}><span>{row.label}</span><strong>{row.value}</strong></div>)}</div><p className="hint"><strong>Crossing Engine 7.1</strong> thêm fast-confirm cho xe nhanh, adaptive cooldown cho lượt quay đầu thật, rescue validation chống ID-jump và Rider-aware Human Guard 2.0 chặn người đi bộ nhưng giữ người đang ngồi/lái xe hai bánh. Tổng xe vẫn là <strong>IN + OUT</strong>; telemetry không còn gọi mọi crossing có gap &gt; 1 frame là “cứu”. Chế độ Phát mượt dùng video native; AI vẫn detect/track/đếm ở nền.</p></article>
      </section>
      ...
      <section className="line-layout" id="cameras">
        <article className="panel line-panel"><div className="panel-head"><div><span className="panel-kicker">ROAD ZONE + COUNTING LINE</span><h2>Khoanh lòng đường và đặt vạch đếm</h2></div><span className={`lock-state ${activePipeline ? 'locked' : ''}`}>{activePipeline ? 'Đang khóa khi AI chạy' : 'Có thể chỉnh'}</span></div>
          ...
          <p className="hint"><strong>Vùng xanh = lòng đường được phép đếm.</strong> V0.5.24 giữ <strong>Hybrid Recall</strong> + <strong>Auto Road-Zone</strong>, <strong>Single-Object Guard</strong> và <strong>Rider-aware Human Guard 2.0</strong> để BUS/TRUCK chồng box chỉ được tính là một phương tiện: khi AI chạy đủ track chuyển động, bấm “AI đề xuất theo luồng xe” để hệ thống tự khoanh phần đường xe thật sự đi qua và đặt vạch gần vuông góc luồng xe. Hybrid Recall: YOLO26 pretrained quét toàn khung để tạo track ổn định, còn best.pt tùy biến kiểm tra lại class tại crossing. Detection chưa có ID vẫn hiện box vàng; chỉ track cắt vạch vàng hợp lệ trong vùng xanh mới được cộng IN/OUT.</p>
        </article>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.24</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.24 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
