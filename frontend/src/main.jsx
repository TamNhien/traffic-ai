...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.31</span></div>
...
function GroundTruthBenchmark({ selectedCameraId, sessions }) {
  ...
  return <section className="benchmark-layout" id="benchmark">
    <article className="panel benchmark-panel">
      <div className="panel-head"><div><span className="panel-kicker">GROUND-TRUTH COUNTING BENCHMARK</span><h2>Đánh dấu xe thật cắt vạch trên chính clip</h2></div><span className="lock-state">V0.5.31</span></div>
    ...
    <main>
      ...
      <section className="content-grid" id="live">
        <article className="panel camera-panel">
          ...
          {activePipeline && (activePipeline.detections_current_frame ?? 0) > 0 && (activePipeline.active_tracks ?? 0) === 0 && <div className="source-status bad"><strong>⚠ YOLO thấy xe nhưng ByteTrack chưa cấp ID</strong><span>V0.5.30 vẫn vẽ box DET màu vàng cho detection chưa có ID. Bộ đếm chỉ tăng khi track ổn định; profile ByteTrack high-recall sẽ cố bám các xe nhỏ/nhanh ở những frame tiếp theo.</span></div>}
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.31</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.30 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
