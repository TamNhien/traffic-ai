...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.27</span></div>
...
function GroundTruthBenchmark({ selectedCameraId, sessions }) {
  ...
  return <section className="benchmark-layout" id="benchmark">
    <article className="panel benchmark-panel">
      <div className="panel-head"><div><span className="panel-kicker">GROUND-TRUTH COUNTING BENCHMARK</span><h2>Đánh dấu xe thật cắt vạch trên chính clip</h2></div><span className="lock-state">V0.5.27</span></div>
    ...
    <main>
      ...
      <section className="content-grid" id="live">
        <article className="panel camera-panel">
          ...
          {activePipeline && (activePipeline.detections_current_frame ?? 0) > 0 && (activePipeline.active_tracks ?? 0) === 0 && <div className="source-status bad"><strong>⚠ YOLO thấy xe nhưng ByteTrack chưa cấp ID</strong><span>V0.5.27 vẫn vẽ box DET màu vàng cho detection chưa có ID. Bộ đếm chỉ tăng khi track ổn định; profile ByteTrack high-recall sẽ cố bám các xe nhỏ/nhanh ở những frame tiếp theo.</span></div>}
        ...
        <article className="panel"><div className="panel-head"><div><span className="panel-kicker">VEHICLE COUNT</span><h2>Theo loại phương tiện · phiên hiện tại</h2></div></div><div className="counting-summary"><div><span>Tổng lượt cắt vạch</span><strong>{sessionPipeline?.total_count ?? 0}</strong></div><div><span>IN</span><strong>{sessionPipeline?.in_count ?? 0}</strong></div><div><span>OUT</span><strong>{sessionPipeline?.out_count ?? 0}</strong></div></div><div className="crossing-quality"><span>Trực tiếp <strong>{sessionPipeline?.direct_crossings ?? 0}</strong></span><span>Nội suy <strong>{sessionPipeline?.interpolated_crossings ?? 0}</strong></span><span>Cứu qua gap <strong>{sessionPipeline?.rescued_crossings ?? 0}</strong></span><span>Fast-confirm <strong>{sessionPipeline?.fast_confirm_rescues ?? 0}</strong></span><span>Bracket-confirm <strong>{sessionPipeline?.bracket_confirm_rescues ?? 0}</strong></span><span>Rescue bị loại <strong>{sessionPipeline?.rescue_validation_rejections ?? 0}</strong></span><span>Class refine <strong>{sessionPipeline?.class_refine_checks ?? 0}</strong></span><span>Refiner miền <strong>{sessionPipeline?.domain_refine_checks ?? 0}</strong></span><span>Refiner chung <strong>{sessionPipeline?.general_refine_checks ?? 0}</strong></span><span>Consensus <strong>{sessionPipeline?.class_consensus_rescues ?? 0}</strong></span><span>Xe đạp cứu <strong>{sessionPipeline?.bicycle_class_rescues ?? 0}</strong></span><span>Xe đạp thấy <strong>{sessionPipeline?.bicycle_tracks_seen ?? 0}</strong></span><span>Xe tải cứu <strong>{sessionPipeline?.truck_class_rescues ?? 0}</strong></span><span>Xe tải thấy <strong>{sessionPipeline?.truck_tracks_seen ?? 0}</strong></span><span>Xe tải cắt vạch <strong>{sessionPipeline?.truck_crossing_tracks ?? 0}</strong></span><span>Video-start <strong>{sessionPipeline?.video_start_rescues ?? 0}</strong></span><span>Human Guard <strong>{sessionPipeline?.human_guard_rejections ?? 0}</strong></span><span>Rider giữ <strong>{sessionPipeline?.rider_guard_rescues ?? 0}</strong></span><span>Guard chờ <strong>{sessionPipeline?.human_guard_pending_crossings ?? 0}</strong></span><span>Guard xác nhận <strong>{sessionPipeline?.human_guard_deferred_commits ?? 0}</strong></span><span>Guard timeout <strong>{sessionPipeline?.human_guard_pending_drops ?? 0}</strong></span></div><div className="vehicle-list">{vehicleRows.map(row => <div className="vehicle-row" key={row.label}><span>{row.label}</span><strong>{row.value}</strong></div>)}</div>{(sessionPipeline?.truck_tracks_seen ?? 0) > 0 && (sessionPipeline?.counts_by_type?.truck ?? 0) === 0 && <div className="source-status"><strong>ℹ AI đã thấy xe tải</strong><span>Đã thấy {sessionPipeline?.truck_tracks_seen ?? 0} track xe tải, nhưng chưa có xe tải nào cắt vạch vàng hợp lệ. “Xe tải thấy” là nhận diện/track; dòng “Xe tải” chỉ tăng khi track thực sự cắt vạch.</span></div>}<p className="hint"><strong>V0.5.27</strong> thêm <strong>Dual Refiner Consensus 4.0</strong>: khi best.pt đang hoạt động, best.pt làm refiner theo miền và YOLO26m làm refiner chung độc lập; chỉ bằng chứng target-matched lặp ở các frame khác nhau mới được consensus cứu xe đạp/xe tải hiếm. <strong>Xe tải thấy</strong> là số track đã từng được nhận diện là truck; <strong>Xe tải cắt vạch</strong> là số track truck đã phát crossing geometry; dòng <strong>Xe tải</strong> vẫn chỉ là event đã đếm. Video local giữ Video-start Rescue; Crossing Engine 7.2 + Transactional Human Guard 2.1 tiếp tục giữ nguyên.</p></article>
      </section>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.27</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.27 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
