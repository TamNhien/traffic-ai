...
const defaultCameraForm = () => ({
  ...
  confidence_threshold: 0.12, line_x1: 0.32, line_y1: 0.59, line_x2: 0.84, line_y2: 0.59,
...
function AnnotationEditor({ dataset, onChanged }) {
  ...
  return <section className="panel annotation-panel" id="annotation">
    <div className="panel-head"><div><span className="panel-kicker">ANNOTATION STUDIO · SMART REVIEW</span><h2>3. Chỉ rà soát ảnh cần thiết trước khi train lại</h2></div><span className="lock-state">V0.5.14</span></div>
...
function App() {
  ...
  const [lineForm, setLineForm] = useState({confidence_threshold:0.12,line_x1:0.32,line_y1:0.59,line_x2:0.84,line_y2:0.59,road_x1:0.20,road_y1:0.16,road_x2:0.80,road_y2:0.16,road_x3:0.96,road_y3:0.98,road_x4:0.04,road_y4:0.98})
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      location: selected.location || '', confidence_threshold: selected.confidence_threshold ?? 0.12,
    ...
    setLineForm({
      confidence_threshold: selected.confidence_threshold ?? 0.12,
      line_x1: selected.line_x1 ?? 0.32, line_y1: selected.line_y1 ?? 0.59,
      line_x2: selected.line_x2 ?? 0.84, line_y2: selected.line_y2 ?? 0.59,
      road_x1:selected.road_x1 ?? 0.20, road_y1:selected.road_y1 ?? 0.16, road_x2:selected.road_x2 ?? 0.80, road_y2:selected.road_y2 ?? 0.16,
      road_x3:selected.road_x3 ?? 0.96, road_y3:selected.road_y3 ?? 0.98, road_x4:selected.road_x4 ?? 0.04, road_y4:selected.road_y4 ?? 0.98
    ...
    <main>
      ...
      <section className="line-layout" id="cameras">
        <article className="panel line-panel"><div className="panel-head"><div><span className="panel-kicker">ROAD ZONE + COUNTING LINE</span><h2>Khoanh lòng đường và đặt vạch đếm</h2></div><span className={`lock-state ${activePipeline ? 'locked' : ''}`}>{activePipeline ? 'Đang khóa khi AI chạy' : 'Có thể chỉnh'}</span></div>
          ...
          <p className="hint"><strong>Vùng xanh = lòng đường được phép đếm.</strong> V0.5.14 cho YOLO/ByteTrack <strong>quét toàn khung hình</strong> để không bỏ xe chỉ vì Road Zone đặt hẹp; nhưng chỉ track cắt vạch vàng hợp lệ bên trong vùng xanh mới được cộng IN/OUT. Vì vậy có thể thấy box ở lề, nhưng xe lề vẫn không được đếm.</p>
        </article>
      ...
      <section className="training-layout" id="training">
        <article className="panel training-panel"><div className="panel-head"><div><span className="panel-kicker">DATASET STUDIO · CLEAN RETRAIN</span><h2>Dataset giao thông Việt Nam</h2></div><span className="lock-state">V0.5.14</span></div>
          <p className="hint"><strong>Train lại từ đầu</strong> nên tạo dataset mới sạch từ video gốc. V0.5.14 mặc định lấy mỗi 15 frame, tối đa 600 ảnh và tự loại frame gần trùng; vì vậy bạn không còn phải mặc định xử lý 1.200 ảnh gần giống nhau.</p>
